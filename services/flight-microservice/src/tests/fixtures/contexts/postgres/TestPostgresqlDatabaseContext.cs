...
/// </summary>
public class TestPostgresqlDatabaseContext : FlightDatabaseContext
{
    /// <summary>
    /// DbSet for Flight entities
    /// </summary>
    public new DbSet<FlightPostgresqlModel> Flights => Set<FlightPostgresqlModel>();

    /// <summary>
    /// DbSet for Airport entities
    /// </summary>
    public new DbSet<AirportPostgresqlModel> Airports => Set<AirportPostgresqlModel>();
    ...
    public TestPostgresqlDatabaseContext(DbContextOptions<FlightDatabaseContext> options)
        : base(options)
    {
    }
    ...
    /// - TEST_POSTGRESQL_DATABASE (default: test_flights)
    ...
    private static string GetConnectionStringFromEnvironment()
    {
        ...
        var port = Environment.GetEnvironmentVariable("TEST_POSTGRESQL_PORT") ?? "5462";
        var database = Environment.GetEnvironmentVariable("TEST_POSTGRESQL_DATABASE") ?? "test_flights";
